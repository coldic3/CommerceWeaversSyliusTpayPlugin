...
final class PaymentCancellerTest extends TestCase
{
    ...
    public function test_it_cancels_a_payment_using_the_new_state_machine_if_present(): void
    {
        ...
        $this->stateMachine->apply($payment, PaymentTransitions::GRAPH, PaymentTransitions::TRANSITION_CANCEL)->shouldBeCalled();
        $this->stateMachineFactory->get(Argument::cetera())->shouldNotBeCalled();

        $this->createTestSubject()->cancel($payment->reveal());
    }
    ...
    private function createTestSubject(): PaymentCancellerInterface
    {
        return new PaymentCanceller($this->stateMachine->reveal(), $this->stateMachineFactory->reveal());
    }
}
